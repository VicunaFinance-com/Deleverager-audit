...
import {IStableDebtToken} from 'contracts/interfaces/IStableDebtToken.sol';
...
contract UiPoolDataProviderV3 is IUiPoolDataProviderV3 {
    ...
    returns (AggregatedReserveData[] memory)
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      reserveData.atokenTotalSupply = IAToken(reserveData.aTokenAddress).totalSupply();
      (
    ...
    returns (UserReserveData[] memory, uint8)
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      userReservesData[i].aTokenBalance = IERC20Detailed(baseData.aTokenAddress).balanceOf(user);
      ...
      if (userConfig.isBorrowing(i)) {
        ...
        userReservesData[i].variableDebt = IERC20Detailed(baseData.variableDebtTokenAddress)
          .balanceOf(user);
