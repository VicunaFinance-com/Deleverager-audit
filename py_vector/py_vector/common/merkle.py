...
class StandardMerkleTree:
    ...
    def __init__(self, leaves, hash_func=keccak):
        ...
        val = {}
        l = {}
        for idx, h in enumerate(hashed_leaves):
            val[h] = val.get(h, 0) + 1
            l[h] = l.get(h, []) + [leaves[idx]]
        print([l[h] for h in hashed_leaves if val[h] > 1])
        print(len(set(hashed_leaves)), len(hashed_leaves))
        print([h for h in val if val[h] > 1])
        leaves_zip = zip(hashed_leaves, leaves)
        assert len(set(hashed_leaves)) == len(hashed_leaves)
